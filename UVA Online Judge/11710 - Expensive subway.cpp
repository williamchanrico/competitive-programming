...
int main(){
	while(scanf("%d %d", &S, &C), S||C){
		...
		for(int a=0;a<C;a++){
			int x, y;
			cin >> inp; x=station[inp];
			cin >> inp; y=station[inp];
			cin >> tmp;
			EdgeList.push_back(make_pair(tmp, make_pair(x, y)));
		}
		cin >> inp; tmp=station[inp];
		...
		if(UF.numDisjointSets()==1) cout << mst_cost << "\n";
		else cout << "Impossible\n";
		EdgeList.clear(); station.clear();
		/*
		for(int a=0;a<C;a++)
